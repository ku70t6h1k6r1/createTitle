...
def ishiguro_distance(str1_a, str2_a):
    """
    INPUT : japanese character or roman character
    """
    if len(str1_a) > len(str2_a) :
        word_l = str1_a
        word_s = str2_a
    else:
        word_l = str2_a
    ...
    mergin = [0 for i in range( len(word_s)-1 )]
    word_l_ext = mergin + word_l + mergin

    score_list = []

    word_s_set = set(word_s)
    for i in range( len(word_l_ext ) - len(word_s) + 1 ) :
        word_l_part = word_l_ext[ i : i + len(word_s) ]
        word_l_part_set = set(word_l_part )
        j_dist = len( word_s_set.intersection(word_l_part_set) ) / len( word_s_set.union(word_l_part_set) )

        if j_dist > 0 :
            cnt = 0
            for j in range( len(word_s) ):
                if word_s[j] == word_l_ext[ i + j ] :
                    cnt += 1

            score_list.append(cnt * 1.0 * j_dist /  len(word_s))
        else:
            score_list.append(0)
...
def ishiguro_distance2(str1_a, str2_a):
    ...
    for i in range( len(word_l_ext ) - len(word_s) + 1 ) :
        ...
        word_l_part_set = set(word_l_part )
        j_dist = len( word_s_set.intersection(word_l_part_set) ) / len( word_s_set.union(word_l_part_set) )
        l_dist = edit_distance(word_l_part, word_s)

        if j_dist > 0 :
            score_list.append(l_dist  * 1.0 * j_dist /  len(word_s))
        else:
            score_list.append(0)
...
def demo2(w1,w2):
    ...
    print("###########")
    print(str(w1).decode('utf-8') , " " , str(w2).decode('utf-8') , " dist1: " ,ishiguro_distance(str_a, str2_a))
    print(str(w1).decode('utf-8') , " " , str(w2).decode('utf-8') , " dist2: " ,ishiguro_distance2(str_a, str2_a))
...
if __name__ == '__main__':
    ...
    demo2("かみきり", "がめら")
    demo2("のうみん", "むうみん")
    demo2("ふしぎのくにのありす", "りあすしきかいがん")
    demo2("しょうじき", "そうじき")
    demo2("しばさきこう", "てばさき")
    demo2("らいおねるりっちー", "りっちーぶらっくもあ")
